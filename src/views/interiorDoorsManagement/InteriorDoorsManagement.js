...
const InteriorDoorManagement = () => {
    ...
    const fetchData = async () => {
        const response = await axios.get(`http://localhost:5000/api/interiorDoors/`);
        setBiFoldDoorsDetails(response.data.data);
    };

    const fetchCategory = async () => {
        const response = await axios.get(`http://localhost:5000/api/category/`);
        const allSubcategory = response.data.data.filter((category) => category.categoryName === 'Interior Doors');
        setSubCategory(allSubcategory[0].subcategories);
    };

    const handleViewClick = async (id) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/interiorDoors/getbyid/${id}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.post("http://localhost:5000/api/interiorDoors/create", data, {
    ...
    const handleDelete = async (id) => {
        ...
        if (isConfirmed) {
            try {
                await axios.delete(`http://localhost:5000/api/interiorDoors/delete/${id}`);
    ...
    const handleUpdate = async () => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/interiorDoors/update/${selectedProduct._id}`, data, {
                headers: { "Content-Type": "multipart/form-data" },
            });
    ...
    const [dimensions, setDimensions] = useState({
        ...
        preHungOptions: { label: "Select Pre Hung Option", data: [] },
    ...
    const handleDimensionSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(
                `http://localhost:5000/api/interiorDoors/add-dimensions/${dimensionId}`,
...
export default InteriorDoorManagement;
