...
const SlidingDoorManagement = () => {
    const [visible, setVisible] = useState(false);
    const [viewModalVisible, setViewModalVisible] = useState(false);
    const [updateModalVisible, setUpdateModalVisible] = useState(false);
    const [biFoldDoorsDetails, setBiFoldDoorsDetails] = useState([]);
    const [biFoldDoorsProduct, setBiFoldDoorsProduct] = useState([]);
    ...
    const fetchData = async () => {
        const response = await axios.get(`http://localhost:5000/api/slidingDoors/`);
        setBiFoldDoorsDetails(response.data.data);
    };

    const fetchCategory = async () => {
        const response = await axios.get(`http://localhost:5000/api/category/`);
        const allSubcategory = response.data.data.filter((category) => category.categoryName === 'Sliding Doors');
        setSubCategory(allSubcategory[0].subcategories);
    };

    const handleViewClick = async (id) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/slidingDoors/getbyid/${id}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.post("http://localhost:5000/api/slidingDoors/create", data, {
    ...
    const handleDelete = async (id) => {
        ...
        if (isConfirmed) {
            try {
                await axios.delete(`http://localhost:5000/api/slidingDoors/delete/${id}`);
    ...
    const handleUpdate = async () => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/slidingDoors/update/${selectedProduct._id}`, data, {
                headers: { "Content-Type": "multipart/form-data" },
            });
    ...
    const [dimensions, setDimensions] = useState({
        ...
        gridOption: { label: "Choose Grid Option", data: [] },
        frameExtrusion: { label: "Choose Frame Extrusion", data: [] },
        color: { label: "Color", data: [] },
        sideTheWindowOpens: { label: "Side the window opens", data: [] },
        lockOption: { label: "Lock Option", data: [] },
        glassOption: { label: "Glass Option", data: [] },
    ...
    const handleDimensionSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(
                `http://localhost:5000/api/slidingDoors/add-dimensions/${dimensionId}`,
    ...
    return (
        <>
            <CCard>
                <CCardHeader>
                    <div className="d-flex justify-content-between align-items-center">
                        <h5>Interior Doors Management</h5>
                ...
                <CCardBody>
                    <CTable striped bordered hover responsive>
                        ...
                        <CTableBody>
                            {biFoldDoorsDetails.map((data, index) => (
                                <CTableRow key={index}>
            ...
            <CModal size='lg' visible={viewModalVisible} onClose={() => setViewModalVisible(false)}>
                ...
                <CModalBody>
                    {biFoldDoorsProduct.productDetails && (
                        <CRow className="align-items-center">
                            <CCol xs={12} md={6} className="mb-3 d-flex justify-content-center">
                                <img
                                    src={biFoldDoorsProduct.productDetails.images[0]}
                                    alt="subcategory"
                                    width="100%"
                            ...
                            <CCol xs={12} md={6}>
                                <div className="text-md-left" style={{ textAlign: 'left' }}>
                                    ...
                                    <p><strong>Product Name:</strong> {biFoldDoorsProduct.productDetails.productName || "N/A"}</p>
                                    <p><strong>Catgory Name:</strong> {biFoldDoorsProduct.productDetails.categoryName || "N/A"}</p>
                                    <p><strong>Sub-Category Name:</strong> {biFoldDoorsProduct.productDetails.subCategory || "N/A"}</p>
                                    <p><strong>Sub Sub-Category Name:</strong> {biFoldDoorsProduct.productDetails.subSubCategory || "N/A"}</p>
                                    <p><strong>Description:</strong> {biFoldDoorsProduct.productDetails.description || "N/A"}</p>
                                    <p><strong>Price:</strong> ${biFoldDoorsProduct.productDetails.price || "N/A"}</p>
                                </div>
                            </CCol>
                        </CRow>
                    )}
                        ...
                        <CCol xs={12}>
                            ...
                            >
                                <div className="card-body">
                                    ...
                                    {biFoldDoorsProduct.dimensions && Object.keys(biFoldDoorsProduct.dimensions).length > 0 ? (
                                        <CRow>
                                            {Object.entries(biFoldDoorsProduct.dimensions)
...
export default SlidingDoorManagement;
