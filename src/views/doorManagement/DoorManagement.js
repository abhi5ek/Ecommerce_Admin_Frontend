...
const DoorManagement = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://44.196.64.110:5000/api/doors/`);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axios.post('http://44.196.64.110:5000/api/doors/add-doors', formDataToSend, {
                headers: {
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            try {
                await axios.delete(`http://44.196.64.110:5000/api/doors/delete-door/${id}`);
                fetchData();
            } catch (error) {
                console.error(error);
            }
    ...
    const handleEditSubmit = async (id) => {
        try {
            ...
            const response = await axios.put(`http://44.196.64.110:5000/api/doors/update-door/${id}`, formDataToSend, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
    ...
    const handleDimesionSubmit = async () => {
        try {
            const response = await axios.post(`http://44.196.64.110:5000/api/doors/add-dimensions/${doorID}`, dimensionFormData, {
